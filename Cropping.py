...
class ImageLoader(tk.Tk):
    def __init__(self):
        ...
        self.ResetButton = ttk.Button(self.frame2, text='Reset', command=self.reset)
        self.ResetButton.grid(row=12, columnspan=2)
...
#-----------------------------functions-----------------------------------------
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.canvas.draw()
                    print('cropped removed as well')
                except ValueError:
                    self.cropped_image.remove()
                    self.canvas.draw()
                    print('only cropped removed')
    ...
    def drawline(self):
        try:
            ...
            self.PreciseCrop.grid(row=2, columnspan=2)
            self.InteractiveRectangle.grid(row=4, columnspan=2)
            self.BetterPreciseCrop['state'] = 'normal'
            self.BetterPreciseCrop.grid(row=3, columnspan=2)
    ...
    def cropper(self):
        ...
        self.SaveImage.grid(row=5, columnspan=2)
